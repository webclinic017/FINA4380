...
valid_c = data_close[data_close.index <= '2018-1-1'].dropna(axis=1, how='all').columns
data_st = data.stack()
...
for c in valid_c:
    ...
    data_is[c] = data_temp[data_temp.index <= "2020-1-1"]
    data_oos[c] = data_temp[data_temp.index > "2020-1-1"]
    data_train[c] = data_temp[data_temp.index <= "2018-1-1"]
    data_test[c] = data_is[c][data_is[c].index >= "2018-1-2"]
    # code to get csv's
    """
